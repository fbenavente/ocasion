...
function treerecurse(&$params, $id = 0, $level = 0, $begin = false)
	{
		...
		if ($level < intval($end_level) || is_null($end_level))
		{
			...
			foreach ($rows as $row)
			{
				...
				{
					$output .= '<li class="cat-item"><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'">'.$row->name.'</a>'.$row->numOfItems;
					treerecurse($params, $row->id, $level + 1);
					$output .= '</li>';
				}
				else
				{
					$output .= '<li class="cat-item"><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'">'.$row->name.'</a>'.$row->numOfItems.'</li>';
				}
			}
			$output .= '</ul>';
		}
