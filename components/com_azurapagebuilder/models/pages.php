...
class AzuraPagebuilderModelPages extends JModelList
{
	...
	/**
	 * Get the master query for retrieving a list of pages subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				'a.id, a.title, a.alias, a.introtext, a.fulltext, ' .
					...
					'a.hits,  a.language, ' . ' ' . $query->length('a.fulltext') . ' AS readmore'
			)
		);
		...
		// Join over the frontpage pages. ?
		if ($this->context != 'com_azurapagebuilder.featured')
		{
		...
		$subquery .= 'WHERE parent.extension = ' . $db->quote('com_azurapagebuilder');
		...
		// Filter by a single or group of pages.
		$pageId = $this->getState('filter.page_id');

		if (is_numeric($pageId))
		{
			$type = $this->getState('filter.page_id.include', true) ? '= ' : '<> ';
			$query->where('a.id ' . $type . (int) $pageId);
		}
		elseif (is_array($pageId))
		{
			JArrayHelper::toInteger($pageId);
			$pageId = implode(',', $pageId);
			$type = $this->getState('filter.page_id.include', true) ? 'IN' : 'NOT IN';
			$query->where('a.id ' . $type . ' (' . $pageId . ')');
		}
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_azurapagebuilder', true);

		// Convert the parameter fields into objects.
		foreach ($items as &$item)
		{
			// $pageParams = new JRegistry;
			// $pageParams->loadString($item->attribs);

			// // Unpack readmore and layout params
			// $item->alternative_readmore = $pageParams->get('alternative_readmore');
			// $item->layout = $pageParams->get('layout');
			...
			// 				$pageArray[$key] = $pageParams->get($key);
			// 			}
			// 			else
			// 			{
			// 				// Otherwise, use the global value
			// 				$pageArray[$key] = $globalParams->get($key);
			// 			}
			// 		}
			// 	}
			...
			// 	{
			// 		$pageParams = new JRegistry;
			// 		$pageParams->loadArray($pageArray);
			// 		$item->params->merge($pageParams);
			// 	}
			// }
			// else
			// {
			...
			if (!$guest)
			{
				$asset = 'com_azurapagebuilder.page.' . $item->id;
			...
			$item->tags->getItemTags('com_azurapagebuilder.page', $item->id);
		}
