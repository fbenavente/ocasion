...
class AzuraPagebuilderViewPages extends JViewLegacy
{
	...
	public function display($tpl = null){
		...
		// Set the submenu
		AzuraPagebuilderHelper::addSubmenu('pages');
	...
	{
		$canDo	= JHelperContent::getActions('com_azurapagebuilder', 'category', $this->state->get('filter.category_id'));
		$user	= JFactory::getUser();

		// Get the toolbar object instance
		$bar = JToolBar::getInstance('toolbar');

		JToolbarHelper::title(JText::_('COM_AZURAPAGEBUILDER_MANAGER_PAGES'), 'pages_page');

		if ($canDo->get('core.create') || (count($user->getAuthorisedCategories('com_azurapagebuilder', 'core.create'))) > 0)
		{
		...
		JToolbarHelper::help('JHELP_COMPONENTS_AZURAPAGEBUILDER_PAGES');

		JHtmlSidebar::setAction('index.php?option=com_azurapagebuilder');
		...
		JHtmlSidebar::addFilter(
			...
			'filter_category_id',
			JHtml::_('select.options', JHtml::_('category.options', 'com_azurapagebuilder'), 'value', 'text', $this->state->get('filter.category_id'))
		);
