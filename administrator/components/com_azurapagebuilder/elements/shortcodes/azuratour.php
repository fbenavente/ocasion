	...
	function azuratour_sc( $atts, $content="" ){
		...
		if(stripos($layout, '_:') !== false){
			...
			if(stripos($layout, ':') !== false){
				$shortcodeTemp = JPATH_THEMES .'/'.JFactory::getApplication()->getTemplate(). '/html/com_azurapagebuilder/plugin/shortcodes_template/'.substr($layout, stripos($playout, ':')+1).'.php';
			}else{
