...
//[Ajax Login form]
if(!function_exists('azuraajaxlogin_sc')) {

	function azuraajaxlogin_sc( $atts, $content="" ) {
		...
		$twofactormethods = ElementParser::getTwoFactorMethods();
		$returnUrl = ElementParser::getReturnURL($itemid);
		...
		if(!empty($styleTextTest)){
			$ajaxloginstyle .= trim($styleText);
		}

		if(!empty($ajaxloginstyle)){
			$ajaxloginstyle = 'style="'.$ajaxloginstyle.'"';
		}
		...
		if(stripos($layout, '_:') !== false){
			...
			if(stripos($layout, ':') !== false){
				...
				$shortcodeTemp = ElementParser::addShortcodeTemplate('azuraajaxlogin');
			}
		}
	...
	ElementParser::add_shortcode( 'AzuraAjaxLogin', 'azuraajaxlogin_sc' );
}