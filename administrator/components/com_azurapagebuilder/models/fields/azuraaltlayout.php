 ...
 * @since       11.1
 */
class JFormFieldAzuraAltlayout extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		// Get the template.
		$template = (string) $this->element['template'];//echo'<pre>';var_dump($template);die;
		...
		$view = preg_replace('#\W#', '', $view);//echo'<pre>';var_dump($view);die;
		...
		if ($extn && $view && $client)
		{
			...
			if (is_dir($component_path) && ($component_layouts = JFolder::files($component_path, '^[^_]*\.xml$', false, true)))
			{
				...
				foreach ($component_layouts as $i => $file)
				{
					...
					$groups['_']['items'][] = JHtml::_('select.option', '_:homepage', 'Home Page');
				}
			}
